...
| Migration | Exact candidate migration and 68/68 pgTAP assertions passed on disposable PGlite PostgreSQL 18.3, including narrow active-account preference access | This pgTAP run uses synthetic pre-existing Edison dependencies; the separate native full-chain rehearsal below is not a hosted Supabase test |
...
| `components/edison/demand-reader.tsx` | `d4a8c7bf823e19ccf7d3fc8cb645701deac84358a06d8a934272c7a17b3608d1` |
| `app/demand.css` | `67d7138c96dd9f9a0b862b0115b89b080318ef0e849133515c5d7e562dbfa9c2` |
| `lib/demand-reader-state.ts` | `340beac5492e968a2764a6d26e569ba498ffebb500382ac905bd2424b1108465` |
| `lib/demand-reader-history.ts` | `528aab4e78148574f0895693120aa8bec2faf065bdd6a018010740485592e5a1` |
| `tests/demand-reader.test.ts` | `e4a80078ba41e821f1082b8ef11c5ed3cd067ed5defb29180ded3f2001b5edaf` |
| `tests/demand-reader-state.test.ts` | `4c69a52fc86e8cfbdaca37f9b043d128192b16121505dfd9d77765efde8d147b` |
| `tests/demand-reader-history.test.ts` | `5814bc8519d322f201aa4a0846085aa91baf91c2599b8b3a6a6baee5972c6ad3` |
...
| `supabase/migrations/20260906000100_on_demand_reading.sql` | `48c8ee0e6a2271ac1546faee2553d27d3e6fa02dd5fd7537181f585fe2a7f400` |
...
Design independently matched the earlier corrected UI hashes and reran 26 focused cases. Its bounded source/unit review closed the listed corrections except same-article leave/reopen position; it also noted the quiet ending Back should remain after Next. Both follow-ups are now implemented, alongside the separate bounded history surface and exact saved-status recovery. The final focused cases cover pages begun before/during Save, both page/exact response orders, monotonic request identity/status, principal changes, cache limits and older-page reload/Back. Root inspected this integration and reran 279 web tests. Design's independent final-source closure and actual rendered/connected acceptance remain separate.
...
3. **CTO:** obtain green full PG17/Supabase CI and verify the actual hosted pooler/Workflow retry/accounting, building on the completed native full-chain and multi-session evidence above. Use existing release authority only once the applicable checks pass; do not rerun historical migrations/publication operators.
