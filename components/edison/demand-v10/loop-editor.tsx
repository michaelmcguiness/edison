...
import { loopDraftChanged, readScopedDraft, saveScopedDraft, type LoopEditDraft } from "./reader-state";
...
export function LoopEditor({ loop, workspaceId, instructions, onSave, onDelete, onClose, onUndo, pendingUndo, status, error: undoError }: {
  ...
  const [draft, setDraft] = useState(() => readScopedDraft(key, saved, (value): value is LoopEditDraft =>
    Boolean(value && typeof value === "object" && "name" in value && typeof value.name === "string" && value.name.length <= 120 &&
  ...
  function update(next: LoopEditDraft) { setDraft(next); saveScopedDraft(key, next); }
  async function submit(deleting: boolean, recover = false) {
    if (lock.current || pendingUndo) return;
    if (attempt.current && !recover) return;
    if (!deleting && draft.name !== loop.title && draft.name.trim().length > 80) { setError("Choose a loop name of 80 characters or fewer."); return; }
    ...
    try {
      ...
      saveScopedDraft(key, currentAttempt.deleting ? saved : currentAttempt.draft);
  ...
  return <section ref={panel} className="demand-floating-panel demand-loop-editor" role="dialog" aria-modal="false" aria-labelledby={`${id}-title`}
    ...
    {confirmDelete ? <><p><strong>{loop.title}</strong> will disappear from your loops. Saved reading, existing reading and conversations will stay in Library.</p><p className="demand-scope-note">Already-started generation is not cancelled by deleting this loop.</p>
